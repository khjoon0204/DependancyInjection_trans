...
모든 종속성을 처음에 생성하는것을 선호합니다, 클라이언트객체는 언제나 유효한 상태임을 확신할 수 있기 때문입니다, 어떤 의존성참조는 null 을 가지고 있는것과는 달리. 그러나 이 방법대로면, 나중에 의존성이 바뀌는 것에 대해 대처하지 못합니다. 때문에 클라이언트를 바꿀 수 없게, 그리고 [thread safe](http:// "Thread safety 는 멀티스레드 코드에 사용가능한 컴퓨터프로그래밍 개념입니다. Thread-safe 코드는, 모든 스레드들이 정상적으로 작동하고 의도치않은 간섭 없이 각 스레드 설계스펙대로 수행할 수 있게 오직 공유데이터구조만 수정합니다 thread-safe data structure 에는 다양한 많은 전략들이 있습니다. ") 하게 만들어야 합니다.
~~~
// Constructor
...
main 메소드에서 수동적인 생성은 이렇게 직접적이지 않고, [builders](http:// "builder pattern 은 객체지향 프로그래밍에서 다양한 객체 생성문제를 유연하게 해결하기 위한 디자인된 패턴 입니다. Builder design pattern 은 복잡한 객체 생성 코드를 분리하는 것이 목적입니다. 이것은 Gang of Four design pattern 중 하나 입니다. "), [factories](http:// "객체지향 프로그래밍에서(OOP), factory 는 다른 객체들을 생성하는 객체 입니다 - 공식적으로 팩토리는 객체의 포로토타입 또는 'new' 된 class 들을 리턴하는 펑션 또는 메소드 입니다. 보다 광범위하게는, 'new'객체를 반환하는 이하과정을 'factory' 라고 하고, factory method 또는 factory function 이라고 합니다. 이것은 OOP의 기본 개념이며 여러 관련 소프트웨어 디자인 패턴의 기초를 형성합니다. "), 또는 다른 [construction patterns](http:// "소프트웨어공학에서 creational design patterns 는 객체 생성 메카니즘을 다루는 디자인패턴 입니다, 상황에 맞는 방법으로 객체를 생성하려고 하는. 객체 생성의 기본 형태는 설계 문제 또는 설계에 복잡함을 가져올 수 있습니다. Creational design patterns 는 이런 객체 생성을 잘 다루면서 문제를 해결 합니다.") 를 포함할 수 있습니다. 이것이 보다 낫고 추상적일 수 있습니다. 객체생성하는 코드가 더이상 어플리케이션에 따라 변하지 않는 대신 전역적으로 사용하는 시점에서 수동의존주입은 framework의존주입으로 연결됩니다.
[Spring](http:// "Spring Framework 는 어플리케이션 프레임워크이고 Java platform 의 역전제어 컨테이너 입니다. 프레임워크의 core는 어느 Java application 에서도 사용할 수 있습니다, Java EE platform 의 경우 웹어플리케이션을 만들기 위한 확장기능이 있습니다. 프레임 워크가 특정 프로그래밍 모델을 강요하지는 않지만 Java 커뮤니티에서 EJB (Enterprise JavaBeans) 모델의 추가 또는 대체로 널리 사용됩니다. 스프링 프레임 워크는 오픈 소스입니다.") 과 같은 프레임웍스들은 이러한 객체들을 생성할 수 있고, 클라이언트에게 참조를 돌려보내기 전에 다같이 연결해 놓습니다. 구체적인 ExampleService 에 대한 사항은 코드에서 설정 데이터 파일로 옮겨집니다.
~~~
...
~~~
위의 예시의 경우 클라이언트와 서비스는 스프링에서 주는 어떤 변화도 겪지 않았습니다. 단순한 [POJOs](http:// "소프트웨어공학에서, plain old Java object(POJO)는 보통의 자바객체 이다, 특별히 어떤 제한이 없고 어떠한 class path가 필요하지 않은. 이 용어는 2000 년 9 월 Martin Fowler, Rebecca Parsons 및 Josh MacKenzie에 의해 만들어졌습니다: '왜 사람들은 그들의 시스템에서 일반적으로 사용하는 객체들에 반대하는지 궁금했고 심플한 객체에 멋진 이름이 결여됐다고 결론지었습니다, 그래서 이름을 붙였고, 잘 지은 것 같습니다.'") 를 유지할 수 있습니다. 이것이 스프링이 그것들의 존재여부는 완전히 신경쓰지않고 서비스와 클라이언트를 연결하는 방법입니다. 만약 클래스에 annotations 이 추가된다면 다를 수 있습니다. 특정한 annotation들과 호출들이 여러 클래스로 퍼지는 것을 관리함으로써만 시스템은 스프링과 느슨한 의존관계를 유지합니다. 시스템이 스프링을 오래 사용하려고 한다면 이것은 중요합니다.
단순히 POJOs 를 선택하는것은 비용없이 유지되지 않습니다. 복잡한 구성파일들을 만들고 유지하는데 들이는 노력보다 단순히 클래스에 annotations를 표시하고 spring이 나머지 일을하게 할 수 있습니다. 이름 또는 타입과 매칭하는 convention을 따른다면 의존성문제는 간단히 해결될 수 있습니다. [convention over configuration](http:// "Convention over configuration 은 프레임웍스를 이용한 소프트웨어 디자인 페러다임 입니다, 프레임워크가 기능하도록하고 개발자가 결정하는 수를 줄이려는. 개념은 David Heinemeier Hansson 이 Ruby on Rails 웹프레임워크의 철학을 설명하기 위해 소개됐지만 'sensible defaults' 같은 초기 아이디어와 유저 인터페이스 디자인에서의 principle of least astonishment 와 관련이 있습니다.  ") 을 선택한다면, 어떤 다른 프레임워크를 리펙토링할 때, 프레임워크 특정 annotations를 삭제하는 것은 크게 문제가 안되고 많은 주입 annotations 가 이제는 표준화 되었습니다.
~~~
import org.springframework.beans.factory.BeanFactory;
import org.springframework.context.ApplicationContext;
...
Assembly comparison
여러가지 injector 구현들(factories, service locators, and dependency injection containers) 의존성 주입에 관한한 크게 다르지 않습니다. 차이점은 어디서 그것들이 사용되는지 입니다. factory 나 service locator 를 클라이언트로부터 떼내어 메인에서 호출하면 메인이 의존주입 컨테이너가 됩니다. 인젝터에 대한 모든 지식을 옮김으로써 외부 세계에 대한 지식이없는 깨끗한 클라이언트가 남게됩니다. 그러나 다른 객체들을 이용하는 어떤 객체도 클라이언트가 될 수 있습니다. 메인을 포함하는 객체도 예외는 아닙니다. 이 메인 객체는 의존성주입을 이용하지 않습니다. 그것은 사실 service locator pattern 을 이용하는 것입니다. 이것은 어쩔 수 없는 것으로 service 구현에 대한 선택이 어딘가에는 있어야하기 때문입니다.
